...
#                LOG.debug("Container ID: %s Name: %s Status: %s", container.id, container.name, container.status)
            ...
            if response is not None and response :
                final_result = response.decode('utf-8')
            LOG.debug("Result from npipe is: %s event: %s", result)
            return final_result
        except docker.errors.DockerException as e:
            LOG.debug("Error connecting to Docker: %s", e)
    ...
    def wait_for_result(self, full_path, event, stdout, stderr, start_timer=None):
        ...
        if self.use_npipe :
            response = self.wait_for_npipe_response(full_path, event, stdout)
        else :
